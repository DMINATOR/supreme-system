...
| `MovePlayerCommand(PlayerState player, float x, float y)` | engine | Sets the player's world-space position (`X`, `Y`) on `PlayerState`. Player must not be null. |
| `EnterRegionCommand(WorldState state, WorldMapGenerator generator, int regionX, int regionY)` | engine | Generates a 3×3 region neighborhood centered on `(regionX, regionY)`. State and generator must not be null. |
